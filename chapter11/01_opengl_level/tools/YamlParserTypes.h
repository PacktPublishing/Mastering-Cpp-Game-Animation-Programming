    ...
    static bool decode(const Node& node, ModelSettings& rhs) {
      ...
      if (Node clipNode = node["idle-walk-run-clips"]) {
        try {
          ...
          Logger::log(1, "%s warning: could not parse idle/walk/run blendings, using empty defaults'\n", __FUNCTION__);
      ...
      if (Node clipNode = node["action-clips"]) {
        try {
          ...
          Logger::log(1, "%s warning: could not parse action clip mappings, using empty defaults'\n", __FUNCTION__);
      ...
      if (Node clipNode = node["action-sequences"]) {
        try {
          ...
          Logger::log(1, "%s warning: could not parse allowed clip order, using empty defaults'\n", __FUNCTION__);
      ...
      if (Node clipNode = node["bounding-sphere-adjustments"]) {
        try {
          ...
          Logger::log(1, "%s warning: could not parse bounding sphere adjustment, using empty defaults'\n", __FUNCTION__);
      ...
      if (Node clipNode = node["head-movement-mappings"]) {
        try {
          ...
          Logger::log(1, "%s warning: could not parse head move clip mappings, using empty defaults'\n", __FUNCTION__);
