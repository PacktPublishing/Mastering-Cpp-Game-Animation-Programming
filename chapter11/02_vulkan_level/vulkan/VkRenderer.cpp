...
bool VkRenderer::init(unsigned int width, unsigned int height) {
  ...
  mWorldBoundaries = std::make_shared<BoundingBox3D>(mRenderData.rdWorldStartPos, mRenderData.rdWorldSize);
  initOctree(10, 5);
...
void VkRenderer::drawLevelAABB(glm::vec4 aabbColor) {
  std::shared_ptr<VkLineMesh> levelLineMesh = mAllLevelAABB.getAABBLines(aabbColor);

  mLineIndexCount += levelLineMesh->vertices.size();
  mLineMesh->vertices.insert(mLineMesh->vertices.end(), levelLineMesh->vertices.begin(), levelLineMesh->vertices.end());
}
