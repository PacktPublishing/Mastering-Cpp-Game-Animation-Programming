...
InstanceNode::InstanceNode(int nodeId) : GraphNodeBase(nodeId) {
...
std::shared_ptr<GraphNodeBase> InstanceNode::clone() {
  return std::make_shared<InstanceNode>(*this);
}

void InstanceNode::draw(ModelInstanceCamData modInstCamData) {
...
void InstanceNode::calculateRotation() {
  if (!mRandomRot) {
    mRotation = mMinRot;
    return;
  }
...
void InstanceNode::calculateSpeed() {
  if (!mRandomSpeed) {
    mSpeed = mMinSpeed;
    return;
  }
...
std::optional<std::map<std::string, std::string>> InstanceNode::exportData() {
...
void InstanceNode::importData(std::map<std::string, std::string> data) {
