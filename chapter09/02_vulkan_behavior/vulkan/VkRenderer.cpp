...
void VkRenderer::updateInstanceSettings(std::shared_ptr<AssimpInstance> instance, graphNodeType nodeType,
  ...
  if (nodeType == graphNodeType::instanceMovement) {
    switch (updateType) {
  ...
  } else if (nodeType == graphNodeType::action) {
...
bool VkRenderer::createAABBLookup(std::shared_ptr<AssimpModel> model) {
  ...
  if (numberOfClips > 0 && numberOfBones > 0 && model->getTriangleCount() > 0) {
    ...
    bufferResized |= ShaderStorageBuffer::checkForResize(mRenderData, mShaderBoneMatrixBuffer, trsMatrixSize);
...
bool VkRenderer::checkForInstanceCollisions() {
  ...
  if (mRenderData.rdCheckCollisions == collisionChecks::boundingSpheres) {
    ...
    for (const auto& collisionInstances : modelToInstanceMapping) {
      ...
      /* resize SSBO if needed */
      bufferResized |= ShaderStorageBuffer::checkForResize(mRenderData, mSphereBoneMatrixBuffer, trsMatrixSize);
...
void VkRenderer::cleanup() {
  ...
  vkFreeDescriptorSets(mRenderData.rdVkbDevice.device, mRenderData.rdDescriptorPool, 1, &mRenderData.rdSphereDescriptorSet);
  vkFreeDescriptorSets(mRenderData.rdVkbDevice.device, mRenderData.rdDescriptorPool, 1, &mRenderData.rdAssimpComputeSphereTransformDescriptorSet);
  vkFreeDescriptorSets(mRenderData.rdVkbDevice.device, mRenderData.rdDescriptorPool, 1, &mRenderData.rdAssimpComputeSphereMatrixMultDescriptorSet);
  vkFreeDescriptorSets(mRenderData.rdVkbDevice.device, mRenderData.rdDescriptorPool, 1, &mRenderData.rdAssimpComputeBoundingSpheresDescriptorSet);
