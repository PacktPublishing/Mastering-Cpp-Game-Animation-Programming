...
void SingleInstanceBehavior::update(float deltaTime, bool triggerRoot) {
  ...
  if (!mPendingNodeEvents.empty()) {
    Logger::log(1, "%s warning: %i events left\n", __FUNCTION__, mPendingNodeEvents.size());
    mPendingNodeEvents.clear();
  }
...
void SingleInstanceBehavior::updateNodeStatus(int pinId) {
  ...
  /* if no child node was found: tell parent node that execution finished */
  if (childIds.empty()) {
