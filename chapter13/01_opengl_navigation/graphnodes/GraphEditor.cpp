...
void GraphEditor::createNodeEditorWindow(OGLRenderData& renderData, ModelInstanceCamData& modInstCamData) {
  if (!mShowEditor) {
    return;
  }
  if (!mBehavior) {
  ...
  ImGui::SetNextWindowSizeConstraints(ImVec2(640, 480), ImVec2(std::numeric_limits<float>::max(), std::numeric_limits<float>::max()));
