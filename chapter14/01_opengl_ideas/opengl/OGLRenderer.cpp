...
bool OGLRenderer::init(unsigned int width, unsigned int height) {
  ...
  if (!mSkyboxShader.loadShaders("shader/skybox.vert", "shader/skybox.frag")) {
    Logger::log(1, "%s: skybox shader loading failed\n", __FUNCTION__);
    return false;
  }
  ...
  mRenderData.rdLevelWireframeMiniMapMesh = std::make_shared<OGLLineMesh>();
...
void OGLRenderer::addExistingInstance(std::shared_ptr<AssimpInstance> instance, int indexPos, int indexPerModelPos) {
  ...
  mModelInstCamData.micAssimpInstancesPerModel[
...
void OGLRenderer::drawLevelCollisionTriangles() {
  if (!mLevelCollidingTriangleMesh->vertices.empty()) {
    mUploadToVBOTimer.start();
...
void OGLRenderer::drawAdjacentDebugTriangles() {
  if (!mLevelGroundNeighborsMesh->vertices.empty()) {
    mUploadToVBOTimer.start();
...
void OGLRenderer::drawInstancePaths() {
  if (!mInstancePathMesh->vertices.empty()) {
    mUploadToVBOTimer.start();
...
void OGLRenderer::runBoundingSphereComputeShaders(std::shared_ptr<AssimpModel> model, int numberOfBones, int numInstances) {
  ...
  /* we MUST set the bone offsets to identity matrices to get the skeleton data */
  std::vector<glm::mat4> emptyBoneOffsets(numberOfBones * numInstances, glm::mat4(1.0f));
  mEmptyBoneOffsetBuffer.uploadSsboData(emptyBoneOffsets);
...
bool OGLRenderer::draw(float deltaTime) {
  ...
  /* draw to framebuffer */
  mFramebuffer.bind();
  /* half the embient light color for the background */
  glm::vec3 clearColor = mRenderData.rdLightSourceColor * mRenderData.rdLightSourceIntensity * 0.5f;
  mFramebuffer.clearTextures(clearColor);
