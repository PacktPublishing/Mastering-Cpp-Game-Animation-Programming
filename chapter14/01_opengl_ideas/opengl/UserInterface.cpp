...
void UserInterface::createSettingsWindow(OGLRenderData& renderData, ModelInstanceCamData& modInstCamData) {
  ...
  if (ImGui::CollapsingHeader("Music & Sound")) {
    ...
    if (iter != playlist.end()) {
      currentPlaylistPos = std::distance(playlist.begin(), iter);
    }
    ...
    if (playlistHasEntries) {
      ...
      if (ImGui::BeginCombo("##MusicCombo",
        playlist.at(currentPlaylistPos).c_str())) {
        for (int i = 0; i < playlist.size(); ++i) {
          const bool isSelected = (currentPlaylistPos == i);
          if (ImGui::Selectable(playlist.at(i).c_str(), isSelected)) {
            currentPlaylistPos = i;
    ...
    if (ImGui::Button("Play")) {
      modInstCamData.micPlayMusicTitleCallbackFunction(playlist.at(currentPlaylistPos));
    }
