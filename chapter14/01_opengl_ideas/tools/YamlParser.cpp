...
YAML::Emitter& operator<<(YAML::Emitter& out, const ModelSettings& settings) {
  ...
  if (settings.msHeadMoveClipMappings.size() > 0 &&
  ...
  if (settings.msFootIKChainNodes.at(0).size() > 0 &&
      settings.msFootIKChainNodes.at(1).size() > 0) {
...
YAML::Emitter& operator<<(YAML::Emitter& out, const BehaviorData& behavior) {
  ...
  if (behavior.bdGraphNodes.size() > 0) {
    out << YAML::Key << "nodes";
    out << YAML::Value;
    out << YAML::BeginSeq;;
...
std::vector<EnhancedBehaviorData> YamlParser::getBehaviorData() {
  std::vector<EnhancedBehaviorData> behaviorData;
  ...
  try {
    for (size_t i = 0; i < behaviorNode.size(); ++i) {
      behaviorData.emplace_back(behaviorNode[i].as<EnhancedBehaviorData>());
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return std::vector<EnhancedBehaviorData>{};
  }
...
std::vector<LevelSettings> YamlParser::getLevelConfigs() {
  ...
  try {
    for (size_t i = 0; i < levelNode.size(); ++i) {
      levelSettings.emplace_back(levelNode[i].as<LevelSettings>());
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return std::vector<LevelSettings>{};
  }
  return levelSettings;
}
...
bool YamlParser::createConfigFile(OGLRenderData renderData, ModelInstanceCamData modInstCamData) {
