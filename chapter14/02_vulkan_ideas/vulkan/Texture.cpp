...
bool Texture::loadTexture(VkRenderData &renderData, VkTextureData &texData, std::string textureFilename,
  ...
  if (!uploadToGPU(renderData, texData, stagingData, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight),
      generateMipmaps, static_cast<uint32_t>(mipmapLevels))) {
...
bool Texture::loadCubemapTexture(VkRenderData& renderData, VkTextureData& texData, std::string textureFilename, bool flipImage) {
  int texWidth;
  int texHeight;
  int numberOfChannels;
  ...
  if (!uploadCubemapToGPU(renderData, texData, stagingData, static_cast<uint32_t>(texWidth / 4), static_cast<uint32_t>(texHeight / 3))) {
    Logger::log(1, "%s error: could not load texture '%s'\n", __FUNCTION__, textureFilename.c_str());
    return false;
  }
...
bool Texture::uploadCubemapToGPU(VkRenderData& renderData, VkTextureData& texData, VkTextureStagingBuffer& stagingData, uint32_t width, uint32_t height) {
  VkImageCreateInfo imageInfo{};
  imageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
  imageInfo.imageType = VK_IMAGE_TYPE_2D;
  imageInfo.extent.width = width;
  ...
  for (uint32_t face = 0; face < 6; ++face) {
    ...
    bufferCopyRegion.imageExtent.width = cubeFaceWidth;
    bufferCopyRegion.imageExtent.height = cubeFaceHeight;
    bufferCopyRegion.imageExtent.depth = 1;

    /* offset into buffer is in bytes! */
    offset = cubeFacePositions[face][0] + (cubeFacePositions[face][1]) * width * 4;
    bufferCopyRegion.bufferOffset = offset * 4;

    /* row length and image height are in texels */
    bufferCopyRegion.bufferRowLength = width * 4;
    bufferCopyRegion.bufferImageHeight = height * 3;
  ...
  vkCmdCopyBufferToImage(uploadCommandBuffer, stagingData.stagingBuffer, texData.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
  ...
  texViewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
  texViewInfo.image = texData.image;
  texViewInfo.viewType = VK_IMAGE_VIEW_TYPE_CUBE;
  texViewInfo.format = VK_FORMAT_R8G8B8A8_SRGB;
  texViewInfo.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  texViewInfo.subresourceRange.baseMipLevel = 0;
  texViewInfo.subresourceRange.levelCount = 1;
  texViewInfo.subresourceRange.baseArrayLayer = 0;
  texViewInfo.subresourceRange.layerCount = 6;
