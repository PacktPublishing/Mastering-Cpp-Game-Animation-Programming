...
ModelAndInstanceData& VkRenderer::getModInstData() {
...
void VkRenderer::addExistingInstance(std::shared_ptr<AssimpInstance> instance, int indexPos) {
...
bool VkRenderer::draw(float deltaTime) {
  ...
  mUploadToUBOTimer.start();
  ShaderStorageBuffer::uploadData(mRenderData, mShaderNodeTransformBuffer, mShaderNodeTransFormData);
