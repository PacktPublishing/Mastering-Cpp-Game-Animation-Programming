...
void AssimpInstance::updateInstancePosition(float deltaTime) {
  ...
  float sinRot = std::sin(glm::radians(mInstanceSettings.isWorldRotation.y)) / 8.0f;
  float cosRot = std::cos(glm::radians(mInstanceSettings.isWorldRotation.y)) / 8.0f;
  mInstanceSettings.isWorldPosition.z += mInstanceSettings.isSpeed.x * cosRot - mInstanceSettings.isSpeed.z * sinRot;
  mInstanceSettings.isWorldPosition.x += mInstanceSettings.isSpeed.x * sinRot + mInstanceSettings.isSpeed.z * cosRot;
...
void AssimpInstance::rotateInstance(float angle) {
  ...
  if (mInstanceSettings.isWorldRotation.y < 0.0f) {
